...
def sup_train(train_loader, model, optimizer, device, gamma, use_unsup_loss):
    model.train()
    total_loss = 0

    for sup_data in train_loader:
        optimizer.zero_grad()
        ...
        if use_unsup_loss:
            out = model.forward_graphrep(sup_data)
            cl_loss = model.contrastive_loss(out, sup_data.y, device)
...
if __name__ == '__main__':
    ...
    for run in range(runs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            _ = sup_train(train_loader, model, optimizer, device, gamma, use_unsup_loss)
